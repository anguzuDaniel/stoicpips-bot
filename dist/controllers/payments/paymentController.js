...
exports.cancelSubscription = exports.handleWebhook = exports.initializePayment = void 0;
...
const handleWebhook = async (req, res) => {
    try {
        ...
        if (event.event === 'charge.completed' && event.data.status === 'successful') {
            ...
            const verifyRes = await axios_1.default.get(`https://api.flutterwave.com/v3/transactions/${transactionId}/verify`, {
                headers: { Authorization: `Bearer ${FLW_SECRET_KEY}` }
            });
            ...
            if (data.status === 'successful') {
                ...
                // Update DB
                const { error } = await supabase_1.supabase.from('profiles').update({
