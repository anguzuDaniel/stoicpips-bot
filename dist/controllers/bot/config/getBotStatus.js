    ...
    try {
        ...
        // Passive Mode: If not running/connected, try to connect briefly to get status
            ...
            if (botConfig) {
                ...
                if (token) {
                    const passiveWS = new DerivWebSocket_1.DerivWebSocket({
                        apiToken: token,
                    ...
                    try {
                        await new Promise((resolve, reject) => {
                            ...
                            passiveWS.once('authorized', (auth) => {
        ...
        if (botState && botState.lastSyncTime && (now - botState.lastSyncTime < CACHE_TTL) && botState.cachedDbStatus) {
            ...
            const { data: dbStatus } = await supabase_1.supabase
                .from("bot_status")
                .select("*")
                .eq("user_id", userId)
                .single();
        ...
        if (!tier) {
            ...
            const { data: profile } = await supabase_1.supabase.from('profiles').select('subscription_tier').eq('id', userId).single();
            tier = profile?.subscription_tier || 'free';
            if (botState)
                botState.subscriptionTier = tier;
        }
        res.json({
