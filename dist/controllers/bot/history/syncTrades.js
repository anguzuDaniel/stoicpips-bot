 ...
 */
const syncTrades = async (userId) => {
    try {
        console.log(`🔄 [${userId}] Starting Trade History Sync...`);
        let botState = botStates_1.botStates.get(userId);
        ...
        if (!localDerivWS || !localDerivWS.getStatus().authorized) {
            ...
            // Fetch Config
            const { data: botConfig } = await supabase_1.supabase.from("bot_configs").select("*").eq("user_id", userId).single();
            if (!botConfig) {
            ...
            localDerivWS = new DerivWebSocket_1.DerivWebSocket({
                apiToken: token,
            ...
            await new Promise((resolve, reject) => {
                ...
                localDerivWS.once('authorized', (auth) => {
        ...
        const tradesToUpsert = transactions.map((t) => {
            // Determine Contract Type
            let contractType = t.contract_type;
            if (!contractType && t.shortcode) {
                if (t.shortcode.includes('CALL') || t.shortcode.includes('MULTUP'))
                    contractType = 'CALL';
                else if (t.shortcode.includes('PUT') || t.shortcode.includes('MULTDOWN'))
                    contractType = 'PUT';
            }
            // Determine Symbol
        ...
        const { error } = await supabase_1.supabase.from('trades').upsert(tradesToUpsert, {
