...
import { DerivCandle, DerivConnectionOptions, SupplyDemandZone, TradingSignal } from "./types";
...
class DerivWebSocket extends EventEmitter {
  ...
  private async executeTrade(signal: TradingSignal) {
    ...
    const isMultiplier = (signal.contract_type as any) === 'MULTUP' || (signal.contract_type as any) === 'MULTDOWN';
    ...
    if (isMultiplier) {
      ...
      if (signal.takeProfit) contractParams.limit_order.take_profit = signal.takeProfit;
      if (signal.stopLoss) contractParams.limit_order.stop_loss = signal.stopLoss;
    } else {
