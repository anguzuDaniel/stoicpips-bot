...
async function runBacktest() {
    ...
    const history = await new Promise<DerivCandle[]>((resolve, reject) => {
        deriv.connect();
    ...
    for (let i = windowSize; i < history.length - 5; i++) { // -5 to allow checking future result (5m expiration)
        ...
        if (signal && signal.action !== "HOLD") {
            ...
            if (dd > results.metrics.maxDrawdown) results.metrics.maxDrawdown = dd;
            ...
            if (status === 'win') {
                ...
                if (signal.contract_type === 'CALL') results.metrics.directionalAccuracy.rise.wins++;
                else results.metrics.directionalAccuracy.fall.wins++;
            } else {
                results.metrics.losses++;
                results.metrics.grossLoss += Math.abs(profit);
            }

            if (signal.contract_type === 'CALL') results.metrics.directionalAccuracy.rise.total++;
            else results.metrics.directionalAccuracy.fall.total++;
        }
    }
